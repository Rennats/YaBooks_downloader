...
def pad_title(title: str) -> str:
    """Дополняет название пробелами до фиксированной длины"""
...
def pad_number(num: int, total: int) -> str:
    """Дополняет номер ведущими нулями до количества разрядов total"""
    width = len(str(total))
...
class YaBooksApp:
    ...
    def parse_input_url(self, url: str) -> Tuple[Optional[str], Optional[str]]:
        """Парсинг входной ссылки или UUID"""
        import re
        url = url.strip()
        if '?' in url:
            url = url.split('?')[0]
    ...
    async def determine_resource_type(self, uuid: str) -> Optional[str]:
        """Попытка определить тип ресурса по UUID"""
        tests = [
    ...
    async def process_single_resource(self, uuid: str, content_type: str):
        ...
        if content_type == "audiobook" and not self.bitrate_set:
            try:
        ...
        console.print(f"[cyan]Загрузка {type_names.get(content_type, content_type)}: {title}{series_info}[/cyan]")
    ...
    async def download_resource(self, resource: dict, content_type: str):
        """Скачивание ресурса в зависимости от типа"""
        try:
    ...
    async def run(self, url: Optional[str] = None):
        ...
        if url:
            ...
            console.print("[yellow]Введите UUID или ссылку на книгу/серию/автора Яндекс.Книг[/yellow]")
            console.print("Примеры:")
            console.print("  Книга:      https://books.yandex.ru/books/UUID")
            console.print("  Аудиокнига: https://books.yandex.ru/audiobooks/UUID")
            console.print("  Комикс:     https://books.yandex.ru/comicbooks/UUID")
            console.print("  Серия:      https://books.yandex.ru/series/UUID")
            console.print("  Автор:      https://books.yandex.ru/authors/UUID")
            value = input("> ").strip()
            if not value:
                return
